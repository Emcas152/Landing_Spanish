...
var sts = [
	/*[0]*/
	'Asi que deseas aplicar con nosotros?... Muy bien, Empecemos!!! ^',
	/*[1]*/
	'Asi que dinos cúal es tu nombre?>',
	/*[2]*/
	'',
	/*[3]*/
	'',
	...
	'',
	/*[5]*/
	'Lo que necesitamos ahora es saber que es lo que deseas._^',
	/*[6]*/
	'Por favor usa el siguiente espacio para expresarte>. Mientras mas especifico seas, mejor entenderemos tus necesidades. asi que adelante.',
	/*[7]*/
	'',
	/*[8]*/
	'...Pero por si acaso... No dejes el país... ;-)_^',
	/*[9]*/
	''
];
...
_your_name_react = function() {
	if (your_name.value != '' && si == 1) {
		...
		sts[2] = 'Muy bien, ' + yourname + '. Veamos que más necesitamos... ah!, Cúal es tu E-mail?>'
		si = 2;
		ii = 2;
...
_email_address_react = function() {
	if (email_address.value != '' && si == 2) {
		...
		sts[3] = 'Excelente, ' + yourname + '. Usaremos el correo ' + emailaddress + ' Para enviarte más información. Ahora. Esto es muy importante. Cúal es tu color favorito?>'
		si = 3;
		ii = 3;
...
_your_color_react = function() {
	if (your_color =='rojo' || your_color =='Rojo') {your_color ='red'};
	if (your_color.value != '' && si == 3) {
		document.getElementById('pin_' + ii).className = 'rest';
		yourcolor = your_color.value.replace(/\s/g, '');		
		if (yourcolor =='rojo' || yourcolor =='Rojo') {yourcolor ='red'} 
			else if(yourcolor =='Azul' || yourcolor =='azul') {yourcolor ='blue'} 
				else if(yourcolor =='Morado' || yourcolor =='morado') {yourcolor ='purple'} 
					else if(yourcolor =='lila' || yourcolor =='Lila') {yourcolor ='#6c4675'}
						else if(yourcolor =='Amarillo' || yourcolor =='amarillo') {yourcolor ='yellow'}
							else if(yourcolor =='Negro' || yourcolor =='negro') {yourcolor ='black'};
		yourcolor = yourcolor.toTitleCase();
		sts[4] = 'Estupendo!. ' + your_color.value + ' Será el color de nuestro diseño._sabes que bromeamos, Cierto?_^'
		si = 4;
		ii = 4;
...
_send_pitch = function() {
	if (type.innerHTML.length == 0) {
		sts[6] = 'Enserio, ' + yourname + '? Nada?';
		_type();
	} else if (type.innerHTML.length < 50 && type.innerHTML.length > 0) {
		sts[6] = 'No puedes hablar en serio, ' + yourname + '. Tu descripcion es mas corta que un Tweet. Escribe al menos 140 letras.';
		_type();
	} else if (type.innerHTML.length < 100 && type.innerHTML.length >= 50) {
		sts[6] = 'Vamos Continua, ' + yourname + '. Aun te quedan ' + (140 - type.innerHTML.length) + ' letras por escribir.';
		_type();
	} else if (type.innerHTML.length < 140 && type.innerHTML.length >= 100) {
		sts[6] = 'Solo un poco más, ' + yourname + '. brindanos ' + (140 - type.innerHTML.length) + ' letras más.';
		_type();
		...
		sts[7] = 'Muy Bien!!, ' + yourname + '. Creo que tenemos lo que necesitamos. Gracias por tu interes en formar parte de nuestra Familia. Estaremos en contacto. Pronto.^';
		si = 7;
		ii = 7;
		_update_step();
		_type();
